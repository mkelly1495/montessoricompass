...
folder are mapped to the application's root namespace, 'MontessoriCompass'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "MontessoriCompass"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'MontessoriCompass' in the same way
as `"MontessoriCompass/sass/src"`.
